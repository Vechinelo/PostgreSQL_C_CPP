...
//-------------------------------------------------------------
// 
// You need to create database "tesdb" before run, with tables:
//  customers (fields: customers_id [PK]integer, fullname text, email text ) 
//  ch (fields: fname text, bfile bytea)
// 
//-------------------------------------------------------------
...
int tApcWorkObj::write(int& anRes)
{
    ...
    std::string strSQL = "TRUNCATE public.customers";
...
int tApcWorkObj::writeBlob(int& anRes)
{
    ...
    std::string strSQL = "TRUNCATE public.ch";
    nRes = spDrv->executeSQL(strSQL);
    if (nRes) {
